...
const FormInput = ({ label, name, type = 'text', required = true, isSelect = false, options = [] }) => {
    ...
    <div className="relative my-4 w-full">
      {isSelect ? (
        <div className="relative">
          <Triangle
            className="absolute top-1/2 -translate-y-1/2 w-3 h-2.5 text-black pointer-events-none rotate-180"
            fill="currentColor"
          />
          ...
          <select
            ...
            className={`peer block w-full appearance-none py-2.5 pl-7 pr-2 text-sm font-mulish text-black bg-transparent border-0 border-b-2 border-[#002A17] focus:outline-none focus:ring-0 focus:text-black focus:border-[#BFFF66]`}
          >
            <option value="" disabled hidden></option>
            {options.map((opt, index) => (
              <option key={index} value={opt.toLowerCase()} className="text-black">
                {opt}
              </option>
            ))}
          </select>
